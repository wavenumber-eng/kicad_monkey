...
def test_project_create_seeds_blank_kicad10_default(tmp_path: Path) -> None:
    """KiCadProject.create() matches KiCad 10's default project structure."""
    prj = KiCadProject.create("My Board", tmp_path)
    raw = prj.to_json()
    assert prj.name == "My Board"
    assert prj.directory == tmp_path
    assert prj.project_path == tmp_path / "My Board.kicad_pro"
    assert raw["meta"] == {"filename": "My Board.kicad_pro", "version": 1}
...
def test_project_constructor_keeps_project_path_positional(tmp_path: Path) -> None:
    """The public dataclass constructor keeps the old project_path first field."""
    project_path = tmp_path / "existing.kicad_pro"
    prj = KiCadProject(project_path)
    assert prj.project_path == project_path
    assert prj.name == ""
    assert prj.directory is None
    assert prj.raw == {}


def test_project_create_classmethod_seeds_writable_aggregate(tmp_path: Path) -> None:
    prj = KiCadProject.create("Demo", tmp_path)
    assert prj.name == "Demo" and prj.directory == tmp_path
    assert prj.get_path("meta.filename") == "Demo.kicad_pro"
...
def test_add_schematic_defaults_title_to_project_name(tmp_path: Path) -> None:
    prj = KiCadProject.create("MyProj", tmp_path)
...
def test_write_project_directory_override(tmp_path: Path) -> None:
    prj = KiCadProject.create("Flat", tmp_path)
    prj.add_schematic()
...
def test_embed_worksheet_into_schematic(tmp_path: Path) -> None:
    ...
    prj = KiCadProject.create("Emb", tmp_path / "out")
    prj.add_schematic()
...
def test_reference_worksheet_when_embed_off(tmp_path: Path) -> None:
    prj = KiCadProject.create("Ref", tmp_path)
    prj.add_schematic()
...
def test_library_tables_and_optional_pcb(tmp_path: Path) -> None:
    prj = KiCadProject.create("Full", tmp_path)
...
def test_write_project_lean_by_default(tmp_path: Path) -> None:
    ...
    prj = KiCadProject.create("Lean", tmp_path)
    prj.add_schematic()
    files = prj.write_project()
...
def test_title_block_metadata(tmp_path: Path) -> None:
    prj = KiCadProject.create("Meta", tmp_path)
    prj.add_schematic(
...
def test_seed_library_tables(tmp_path: Path) -> None:
    prj = KiCadProject.create("Libs", tmp_path)
    prj.add_schematic()
...
def test_constructor_name_directory_does_not_seed_project_json(tmp_path: Path) -> None:
    prj = KiCadProject(name="Demo", directory=tmp_path)
    with pytest.raises(ValueError, match="KiCadProject.create"):
        prj.write_project()
...
def test_pcb_accepted_by_kicad_cli(tmp_path: Path) -> None:
    ...
    prj = KiCadProject.create("PcbCli", tmp_path)
    prj.add_schematic()
    prj.add_pcb()
    files = prj.write_project()
...
def test_schematic_accepted_by_kicad_cli(tmp_path: Path) -> None:
    ...
    if cli is None:
        pytest.skip("kicad-cli not found")
    prj = KiCadProject.create("CliCheck", tmp_path)
