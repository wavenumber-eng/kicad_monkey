...
def _a1_netlist(*, source: str = "demo.kicad_sch", tool: str = "kicad_cruncher") -> str:
    return (
        '(export (version "E") '
        f'(design (source "{source}") (date "") (tool "{tool}")) '
...
def _a1_result(
    ...
    *,
    ...
    text = _a1_netlist() if netlist is None else netlist
    ...
    return json.dumps(
        {
            ...
            "kicad_netlist": text,
            "kicad_netlist_bytes": str(len(encoded)),
            "kicad_netlist_sha256": hashlib.sha256(encoded).hexdigest(),
        }
    ).encode()
...
def test_source_snapshot_digest_matches_the_cross_language_a1_algorithm() -> None:
    sources = [
        (Path("demo.kicad_pro"), "project", b"{}\n"),
        (Path("root.kicad_sch"), "schematic", b"abc"),
    ]
    manifest = {
    ...
    assert _source_fingerprint(manifest, sources) == (
        "db9c8920a72e1a3383d928ff74cb892455d0e78d231476b0aa417003d9d6a044"
    )
...
def test_source_bound_design_facts_validate_identity_metadata_and_provenance(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    graph = build_compiled_schematic_graph(_write_design(tmp_path)).to_json()
    executable = tmp_path / "native.exe"
    executable.touch()
    digest = "a" * 64
    calls: list[str] = []
    ...
    def fake_run(_executable, command, _request, **_kwargs):
        ...
        return _a1_result(graph, digest=digest)

    monkeypatch.setattr("kicad_monkey.kicad_native._run_native_command", fake_run)
    handshake = kicad_native_handshake_a2(executable=executable)
    ...
    assert handshake["operations"] == ["design-facts", "render-svg", "design-facts-a1"]
    assert calls == ["handshake-a2", "handshake-a2", "design-facts-a1"]
    assert result.design_fingerprint == digest
    assert result.source_snapshot_sha256 == digest
    assert result.resource_profile == "design-facts-bounded-a1"
    assert result.kicad_netlist_bytes == len(result.kicad_netlist.encode("utf-8"))
    assert result.kicad_netlist_sha256 == hashlib.sha256(
        result.kicad_netlist.encode("utf-8")
    ).hexdigest()


@pytest.mark.parametrize(
    ("result_digest", "netlist", "expected"),
    [
        ("b" * 64, _a1_netlist(), "source snapshot digest"),
        ("a" * 64, _a1_netlist(tool="wrong"), "tool metadata"),
        (
            "a" * 64,
            _a1_netlist().replace("(nets))", "(nets) (nets))"),
            "top-level blocks",
        ),
    ],
)
def test_source_bound_design_facts_reject_wrong_identity_or_structure(
    tmp_path: Path,
    ...
    def fake_run(_executable, command, _request, **_kwargs):
        ...
        return _a1_result(graph, digest=result_digest, netlist=netlist)
...
def test_design_bundle_is_explicit_and_uses_relative_slots(
    ...
    monkeypatch.setattr("kicad_monkey.kicad_native.native_design_facts_a1", fake_design_facts)
