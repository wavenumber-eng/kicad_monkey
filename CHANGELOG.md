...
- Add a new-project assembly surface to `KiCadProject`: construct a writable
  blank project with `KiCadProject(name, directory)` (or `KiCadProject.create`),
  attach pieces through the object model with `add_schematic`, `set_worksheet`,
  `add_pcb`, `add_symbol_library` / `add_footprint_library` /
  `ensure_library_tables`, and write the whole folder with `write_project`.
